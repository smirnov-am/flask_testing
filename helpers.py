...
import subprocess
import time
#####    ekekekkkkkkkkkkkkdasdasdkeqweqwekkkkkkkkkkkkkkkeeekkerekrwdsadasdekrwwwwwsdfsdfewqeqwfdsfasdfsf
import hashlib
import iterltools
import unidiff
import yaml
...
def get_config(repo, base_branch, after_commit_hash):
    """
    Get .turbot.yml config file from the repository and return
    ...
    # Configuration file
    query = f"https://raw.githubusercontent.com/{repo}/{base_branch}/.turbot.yml"
    r = utils.query_request(query)

    if r.status_code == 200:
        new_config_text = r.text
    else:  # Try to look for a config in the head branch of the Pull Request
        new_query = f"https://raw.githubusercontent.com/{repo}/{after_commit_hash}/.turbot.yml"
        r_new = utils.query_request(new_query)
        if r_new.status_code == 200:
...
def comment_permission_check(ghrequest):
    ...
    #     if old_comment["user"]["id"] == 24736507:  # ID of @turbot
...
def create_pr(ghrequest):
    ...
    request_json = {
        ...
        "head": f"turbot:{ghrequest.new_branch}",
